...
```
SessionPanel/
├── SessionList.tsx          # List of previous sessions
├── ActiveSession.tsx        # Live session chat interface
├── SessionMessage.tsx       # Individual chat message
├── NewSessionButton.tsx     # Start new session control
└── TranscribeButton.tsx     # Trigger AI processing
```
...
```
KnowledgePanel/
├── SearchAndFilter.tsx      # Search bar and category filters
├── DetailItemsGrid.tsx      # Tile/grid view of Detail Items
├── DetailItemCard.tsx       # Individual Detail Item preview
├── DetailItemModal.tsx      # Full Detail Item information
└── RelationshipLinks.tsx    # Cross-reference navigation
```
...
```
Layout/
├── SplitLayout.tsx          # Main two-panel interface
├── CampaignHeader.tsx       # Campaign info and navigation
└── UserPresence.tsx         # Show active users in session
```
...
- **Absolute Paths**: Always use absolute paths defined in `tsconfig.json` instead of relative paths or `~/*`
