...
export function useDetailItems(campaignId: string) {
  ...
  const query = useQuery({
    queryKey: ["detail-items", campaignId, filters],
    queryFn: () => getDetailItems(campaignId, filters),
    staleTime: 1000 * 60, // 1 minute
  });

  // Real-time subscription for live updates
  useEffect(() => {
    const subscription = subscribeToDetailItems(campaignId, (detailItem) => {
  ...
  }, [campaignId, filters, queryClient]);
